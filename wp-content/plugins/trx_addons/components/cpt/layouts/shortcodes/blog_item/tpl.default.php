...
$need_wrap = $args['position'] != 'static'
				...
				&& !trx_addons_sc_stack_check('show_layout_sop');
	...
	if ($args['type'] == 'title') {
		...
		if ( $is_preview ) {
			...
			if ( has_excerpt() ) {
				...
				the_content( '' );
			} else if ( in_array( get_post_format(), array( 'link', 'aside', 'status' ) ) ) {
				the_content();
			} else if ( 'quote' == get_post_format() ) {
				$quote = trx_addons_get_tag( get_the_content(), '<blockquote>', '</blockquote>' );
				if ( ! empty( $quote ) ) {
		...
		if ( $is_preview ) {
			$args['meta_parts'] = str_replace(
									'counters',
									is_array($args['counters']) ? implode(',', $args['counters']) : $args['counters'],
									is_array($args['meta_parts']) ? implode(',', $args['meta_parts']) : $args['meta_parts']
									);
			...
			$post_meta = trx_addons_sc_show_post_meta('sc_layouts_meta', apply_filters('trx_addons_filter_show_post_meta', array(
