...
if ( !function_exists( 'trx_addons_cpt_layouts_sop_add_row_params' ) ) {
	...
	function trx_addons_cpt_layouts_sop_add_row_params($fields, $post_id, $args) {
		...
		$fields['row_hide_unfixed'] = array(
			'name'        => esc_html__( 'Hide this row on unfix', 'trx_addons' ),
		...
		$fields['hide_on_wide'] = array(
			'name'        => esc_html__( 'Hide on wide', 'trx_addons' ),
			'label'        => esc_html__( 'Hide', 'trx_addons' ),
		...
		$fields['hide_on_singular'] = array(
			'name'        => esc_html__( 'Hide on single posts', 'trx_addons' ),
			'label'        => esc_html__( 'Hide', 'trx_addons' ),
		...
		$fields['hide_on_other'] = array(
			'name'        => esc_html__( 'Hide on other pages', 'trx_addons' ),
			'label'        => esc_html__( 'Hide', 'trx_addons' ),
...
if ( !function_exists( 'trx_addons_cpt_layouts_sop_row_style_attributes' ) ) {
	...
	function trx_addons_cpt_layouts_sop_row_style_attributes($attributes, $style) {
		...
		if ( !empty($style['hide_on_wide']) && !trx_addons_is_inherit($style['hide_on_wide']) )
			$attributes['class'][] = 'sc_layouts_hide_on_wide';
		...
		if ( !empty($style['hide_on_singular']) && !trx_addons_is_inherit($style['hide_on_singular']) )
			$attributes['class'][] = 'sc_layouts_hide_on_singular';
		if ( !empty($style['hide_on_other']) && !trx_addons_is_inherit($style['hide_on_other']) )
			$attributes['class'][] = 'sc_layouts_hide_on_other';
		return $attributes;
	}
}
...
if ( !function_exists( 'trx_addons_cpt_layouts_sop_sc_wrap' ) ) {
	...
	function trx_addons_cpt_layouts_sop_sc_wrap($classes, $widget_class, $instance, $widget_info) {
		if (trx_addons_sc_stack_check('show_layout')						// Wrap shortcodes in the headers and footers
			...
			if (!empty($instance['hide_on_wide']) && empty($instance['mobile_button']))
				$classes[] = 'sc_layouts_hide_on_wide';
...
if ( !function_exists( 'trx_addons_cpt_layouts_sop_layout_content' ) ) {
	add_filter('trx_addons_filter_sc_layout_content', 'trx_addons_cpt_layouts_sop_layout_content', 11, 2);
	function trx_addons_cpt_layouts_sop_layout_content($content, $post_id) {
		...
		if ( strpos($content, '[vc_row')===false && trx_addons_exists_sop() && get_post_meta( $post_id, 'panels_data', true ) ) {
			trx_addons_sc_stack_push('show_layout_sop');
...
// Check if specified post built with SiteOrigin Panels
if ( !function_exists( 'trx_addons_cpt_layouts_sop_post_built_in' ) ) {
	add_filter( 'trx_addons_filter_post_built_in', 'trx_addons_cpt_layouts_sop_post_built_in', 10, 2 );
	function trx_addons_cpt_layouts_sop_post_built_in($builder, $post_id=0) {
		if ($post_id == 0) $post_id = get_the_ID();
		if ( trx_addons_exists_sop() && $post_id > 0 && get_post_meta( $post_id, 'panels_data', true ) ) {
			$builder = 'sop';
		}
		return $builder;
	}
