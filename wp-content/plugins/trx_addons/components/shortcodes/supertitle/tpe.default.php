			...
			if (item.text !== '') {
				...
				#>><#
					...
					if (item.color !== '' && item.color2 !== '') {
						print('<span class="trx_addons_text_gradient" style="background:' + item.color + ';background:linear-gradient(' + Math.max(0, Math.min(360, item.gradient_direction.size)) + 'deg,' + (item.color2 ? item.color2 : 'transparent')  + ',' + item.color + ');">'
									+ item.text
								+ '</span>');
					} else {
						print(item.text);
