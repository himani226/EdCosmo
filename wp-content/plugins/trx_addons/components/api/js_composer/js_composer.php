...
if ( !function_exists( 'trx_addons_vc_edit_post_link' ) ) {
	...
	function trx_addons_vc_edit_post_link($link, $id=0, $text='') {
		return str_replace('vc_inline-link', 'post_counters_item post_counters_edit vc_inline-link', $link);
	}
}
...
if ( !function_exists( 'trx_addons_vc_importer_check_options' ) ) {
	if (is_admin()) add_filter( 'trx_addons_filter_import_theme_options', 'trx_addons_vc_importer_check_options', 10, 4 );
