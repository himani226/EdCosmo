	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="yolox_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.yolox_post_editor' );
					if ( link.length > 0 ) {
						yolox_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.yolox_post_editor', function(e) {
					yolox_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('yolox_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function yolox_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
