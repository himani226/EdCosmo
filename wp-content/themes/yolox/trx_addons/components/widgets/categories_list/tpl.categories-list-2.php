                ...
                <div class="categories_list_item">
                    ...
                    if ($show_thumbs) {
                        ...
                        $src = empty($image)
                            ...
                            : trx_addons_add_thumb_size($image, yolox_get_thumb_size($columns > 3 ? 'cat-list' : 'medium'));
                        $attr = trx_addons_getimagesize($src);
                        ?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category icon', 'yolox'); ?>">
                        </div><?php
                    }
                    ?><div class="categories_list_title"><?php


                        ?>
                        <div class="cat_title_inner_border"><?php $term = get_term_by( 'id', $cat->term_id, 'category' );
                            if ($term) {
                                $icon = apply_filters('trx_addons_extended_taxonomy_name', $term->term_name, $term);
                                yolox_show_layout($icon);
