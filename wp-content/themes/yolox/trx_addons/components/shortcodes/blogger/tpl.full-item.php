...
if (!empty($post_link)) {
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
									...
									apply_filters('trx_addons_filter_args_featured', array(
														...
														'hover' => 'zoomin',
                                                        'post_info' => '<div class="post_meta_item post_categories">'
                                                            . '<span class="post_categories">'.trx_addons_get_post_categories().'</span>'
                                                            . '</div>',
														'no_links' => empty($post_link),
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', yolox_get_thumb_size($args['columns'] > 2 ? 'blogger-med' : 'blogger-big'), 'blogger-full')
														), 'blogger-full')
								);
