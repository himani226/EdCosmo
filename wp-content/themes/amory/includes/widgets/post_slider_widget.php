...
class category_select_slider_posts extends WP_Widget {
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'category_select_slider_posts-widget', 'Amory - Premium Posts with slider', $widget_ops, '' );
	}
			...
			<li>
				<div class="widgett">		
					...
					if ( has_post_thumbnail() ){
						$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'amory-postBlock', false);
						$image = $image[0];}	
					?>			
					<div class="imgholder">
						<a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>">
							<?php if (has_post_thumbnail( get_the_ID() )) echo amory_getImage(get_the_id(),'amory-widget') ;?>		
						</a>
					</div>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Amory Post slider', 'number' => 3, 'slider' => 2, 'excerpt' => 0, 'n_excerpt' => 10,'category' =>'');
