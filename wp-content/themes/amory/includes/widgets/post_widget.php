...
class category_select_posts extends WP_Widget {
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'category_select_posts-widget', 'Amory - Premium Posts', $widget_ops, '' );
	}
		...
		<div class="widgett <?php if ( has_post_format( 'quote' , get_the_id())) echo 'quote-widget';?> <?php if ( has_post_format( 'link' , get_the_id())) echo 'link-widget';?>">		
			...
			<div class="link-category">
				<div class="blogpostcategory">
					...
					<?php if(amory_getImage(get_the_id(), 'amory_blog') != '') { ?>	
					...
					<div class="blogimage">		
						<a href="<?php echo esc_url($link) ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>"><?php echo amory_getImage(get_the_id(), 'amory_blog'); ?></a>
					</div>
					<?php } ?>												
				</div>
			</div>
			...
			if ( !get_post_format() || has_post_format( 'gallery' , get_the_id())) {
				...
				if ( has_post_thumbnail() ){
					$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()),'amory-postBlock');
					$image = $image[0];}	
					?>			
					...
					<div class="wttitle"><h4><a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>"><?php the_title(); ?></a></h4></div>
					<?php if(!empty($instance['excerpt']) && !(has_post_format( 'link' , get_the_id()))) { ?>
					<div class="pmc-excerpt"><?php echo wp_trim_words(strip_shortcodes(get_the_content()), $instance['n_excerpt'],'...')  ?></div>
					<?php } ?>	
			<?php } ?>
		</div>	
